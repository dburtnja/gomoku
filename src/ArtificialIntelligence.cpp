...
//    }
    ...
    for(auto & element: tmp_vector) {
        ...
        mainBoard.putStoneOnBoard(x_cor, y_cor, AI_PLAYER, 1);
        int tmp = minimaxAlphaBeta (mainBoard, REC_DEPT, HUMAN_PLAYER, INT_MIN, INT_MAX, x_cor,y_cor);
...
int ArtificialIntelligence::minimaxAlphaBeta(GomokuMainBoard & mainBoard, int depth, int player, int alpha, int beta, int x, int y)
{
    int value = 0;
    mainBoard.recs++;
    ...
    if (depth == 0)
    {
        return evaluation(mainBoard, player);
...
//    }
    if (player == AI_PLAYER)
    {
        ...
        for (auto & element: tmp_vector)
        {
            ...
            alpha = std::max(minimaxAlphaBeta(mainBoard, depth - 1, HUMAN_PLAYER, alpha, beta, x_cor, y_cor), alpha);
    ...
    else
    {
        ...
        for (auto & element: tmp_vector)
        {
            ...
            mainBoard.putStoneOnBoard(x_cor, y_cor, player, depth);
            beta = std::min(minimaxAlphaBeta(mainBoard, depth - 1, AI_PLAYER, alpha, beta, x_cor, y_cor), beta);
